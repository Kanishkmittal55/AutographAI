...
const MergedUnstructuredImport: React.FC = () => {
  ...
  const predefinedPrompts = [
    "Dummy prompt for use case A...",
    "Another sample prompt for use case B...",
    "A third dummy prompt for use case C..."
  ];
  const [costControl, setCostControl] = useState("$0.00");
  const [llmInteractions, setLlmInteractions] = useState("0");
  ...
  return (
    <div className="h-screen bg-gray-900 text-white">
      ...
      >
        ...
        <div className="bg-gray-800 p-4 shadow-lg flex flex-col h-full">
        <h2 className="text-lg font-bold mb-2">LLM Interactions</h2>
        <p className="text-sm text-gray-300 mb-2">
          Use the prompt creator tool to better analyse and engineer your prompts.
          Then one can also test the prompt and its output on a dummy chunk(s) of data.
        </p>
        <div className="flex space-x-2 mb-2">
          {predefinedPrompts.map((prompt, index) => (
            <button
              key={index}
              onClick={() => setMainPromptInput(prompt)}
              className="w-10 h-10 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center text-xs text-gray-100"
            >
              {`P${index + 1}`}
            </button>
          ))}
        ...
        <textarea
          ...
          className="flex-1 bg-gray-900 text-gray-100 p-2 rounded resize-none overflow-auto"
        />
      </div>
        ...
        <div className="bg-gray-800 p-4 shadow-lg flex flex-col relative">
          ...
          <div className="mb-2 flex flex-col">
            <h2 className="text-lg font-bold text-blue-400">Cost Control </h2>
            <input
              type="text"
              value={costControl}
              readOnly
              className="bg-gray-900 text-blue-400 text-lg font-bold p-2 rounded"
            />
            <h2 className="text-lg font-bold text-blue-400">LLM Interactions</h2>
            <input
              type="text"
              value={llmInteractions}
              readOnly
              className="bg-gray-900 text-blue-400 text-lg font-bold p-2 rounded"
            />
          </div>
